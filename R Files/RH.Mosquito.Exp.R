...
mosquito.pd.med <- mosquito.pd %>% 
  group_by(Species, Temp) %>% 
  summarize(med = median(Trait), .groups = "drop")

# Plot new data
ggplot(mosquito.pd, aes(y = Trait, x = Temp)) +
  geom_point() +
  geom_point(data = mosquito.pd.med, aes(x = Temp, y = med, color = Species),
             shape = 21, fill = "white", stroke = 1.3, size = 2.5) +
  facet_wrap(~ Species) +
  labs(title = "Temperature by Trait for Each Factor Level",
       x = "Temperature",
       y = "Development Time") +
...
# m.gd.full <- gelman.diag(as.mcmc.list(m.one.data.pd.fit.mcmc), autoburnin = FALSE)
#
...
ggsave("mosquito.full.plot.png", plot = m.p.full.pd, width = 8, height = 4)
...
save(m.one.data.pd.fit,
     m.one.data.pd.fit.DIC,
     file = "m.one.data.pd.fit.exp.RData")
...
save(a.aegypti.pd.fit.DIC,
     a.aegypti.pd.fit,
     file="m.a.aegypti.pd.fit.exp.RData")
...
save(a.albopictus.pd.fit,
     a.albopictus.pd.fit.DIC,
     file="m.a.albopictus.pd.fit.exp.RData")
...
save(a.camptorhynchus.pd.fit.DIC,
     a.camptorhynchus.pd.fit,
     file="m.a.camptorhynchus.pd.fit.exp.RData")
...
save(a.krombeini.pd.fit.DIC,
     a.krombeini.pd.fit,
     file="m.a.krombeini.pd.fit.exp.RData")
...
# 
save(a.notoscriptus.pd.fit.DIC,
     a.notoscriptus.pd.fit,
     file="m.a.notoscriptus.pd.fit.exp.RData")
...
big.grid.m <- grid.arrange(
  ...
  m.p.a.albopictus.pd + labs(title = "Aedes albopictus", x = " ", y = " ") +
    ...
    coord_cartesian(ylim = c(0, 150)),
  m.p.a.camptorhynchus.pd + labs(title = "Aedes camptorhynchus", x = " ", y = " ") +
    theme(plot.title = element_text(size = 14, hjust = 0.5),
          axis.text.x = element_blank(), axis.title.y = element_blank(),
          axis.text.y = element_blank()) +
    coord_cartesian(ylim = c(0, 150)),
  m.p.a.jj.pd + labs(title = "Aedes japonicus japonicus", x = " ", y = "Development Time in Days") +
    theme(plot.title = element_text(size = 14, hjust = 0.5),
  ...
  m.p.a.krombeini.pd + labs(title = "Aedes krombeini", x = " ", y = " ") +
    theme(plot.title = element_text(size = 14, hjust = 0.5),
          ...
          axis.text.x = element_text(size = 14)) +
    coord_cartesian(ylim = c(0, 150)),
  m.p.a.notoscriptus.pd + labs(title = "Aedes notoscriptus", y = " ") +
    theme(plot.title = element_text(size = 14, hjust = 0.5),
          axis.title.y = element_blank(),
          axis.text.y = element_blank(),
          axis.text.x = element_text(size = 14)) +
...
y_label.m <- textGrob("Development Time in Days", rot = 90, gp = gpar(fontsize = 14))

# Combine y-axis label and grid side-by-side with some margin for y-label
m.ind.species.pd <- arrangeGrob(
  y_label.m, big.grid.m,
  ncol = 2,
...
ggsave("mosquito.species.ind.plot.png", m.grid.ind.species.pd, width = 8, height = 4)
...
model {
  ...
  # mu.a ~ dexp(10)            # Mean for species effects Topt
  mu.c ~ dgamma(10, 1)
  mu.Topt ~ dexp(0.001)
  mu.a ~ dexp(10)
  ...
  for (j in 1:m.n.species.pd) {
    ...
    mu.species[j] ~ dgamma(mu.c, mu.c.rate)
    mu.species.Topt[j] ~ dexp(mu.Topt)
    mu.species.a[j] ~ dexp(mu.a)

  }
  # Likelihood
  for (i in 1:m.N.obs.pd) {
    # mu[i] <- mu.species.a[m.species.pd[i]] * (mu.species.Topt[m.species.pd[i]] - temp[i])^2 + mu.species[m.species.pd[i]]
    mu[i] <- mu.species.a[m.species.pd[i]] * exp(- mu.species.Topt[m.species.pd[i]] * temp[i]) + mu.species[m.species.pd[i]]
    trait[i] ~ dnorm(mu[i], tau1) T(0, )
  }}
...
# Parameters to monitor
parameters <- c("mu.a", "mu.c", "mu.species", "mu.c.rate", 
                "tau1", "mu.Topt", "mu.species.Topt", "mu.species.a")
...
save(m.species.pd.fit,
     m.species.pd.fit.DIC,
     file = "m.species.pd.fit.exp.RData")
...
# Parameters to monitor
parameters <- c("mu.a", "mu.c", "mu.species", "species.tau", "mu.c.rate",
                "tau1", "mu.Topt", "mu.species.Topt", "mu.species.a")
...
save(m.species.pd.tau.fit,
     m.species.pd.tau.fit.DIC,
     file = "m.species.pd.tau.fit.exp.RData")

rm(m.species.pd.tau.fit)
...
model {
  ...
  # mu.a ~ dexp(10)            # Mean for climate effects Topt
  mu.c ~ dgamma(10, 1)
  mu.Topt ~ dexp(0.001)
  mu.a ~ dexp(10)
  mu.c.rate ~ dgamma(2, 1)
  ...
  for (i in 1:m.N.obs.pd) {
    ...
    mu[i] <- mu.climate.a[m.climate.pd[i]] * exp(- mu.climate.Topt[m.climate.pd[i]] * temp[i]) + mu.climate[m.climate.pd[i]]
...
save(m.climate.pd.fit,
     m.climate.pd.fit.DIC,
     file = "m.climate.pd.fit.exp.RData")
...
model {
  ...
  # mu.a ~ dexp(10)            # Mean for size effects Topt
  mu.c ~ dgamma(10, 1)
  mu.Topt ~ dexp(0.001)
  mu.a ~ dexp(10)
  mu.c.rate ~ dgamma(2, 1)
  ...
  for (i in 1:m.N.obs.pd) {
    ...
    mu[i] <- mu.size.a[m.size.pd[i]] * exp(- mu.size.Topt[m.size.pd[i]] * temp[i]) + mu.size[m.size.pd[i]]
...
parameters <- c("mu.a", "mu.c", "mu.size", "size.tau",
                "tau1", "mu.Topt", "mu.size.Topt", "mu.size.a")
...
# 
# Get posterior samples
m.size.rm.samp <- as.matrix(m.size.pd.rm.fit.mcmc)
# Get column indices
m.a_size.rm <- grep("^mu\\.size\\.a\\[", colnames(m.size.rm.samp))
m.Topt_size.rm <- grep("^mu\\.size\\.Topt\\[", colnames(m.size.rm.samp))
m.c_size.rm <- grep("^mu\\.size\\[", colnames(m.size.rm.samp))
# Generate temperature sequence 
m.temp_seq <- seq(min(mosquito.pd$Temp), max(mosquito.pd$Temp), by = 0.1)
# Prepare a list to store data frames
m.size.rm_dfs <- list()
for (j in 1:m.n.size.pd) {
  # Retrieve parameter vectors for size j
  m.a_samps <- m.size.rm.samp[, m.a_size.rm[j]]
  m.Topt_samps <- m.size.rm.samp[, m.Topt_size.rm[j]]
  m.c_samps <- m.size.rm.samp[, m.c_size.rm[j]]
  # For each temperature in temp_seq, evaluate across ALL posterior samples
  m.size.rm_eval <- sapply(m.temp_seq, function(temp) {
    m.a_samps * exp(- m.Topt_samps * temp) + m.c_samps
  })
  # Compute pointwise posterior means and HDIs
  m.mean_curve.size.rm <- apply(m.size.rm_eval, 2, mean)
  m.size.rm_dfs[[j]] <- m.mean_curve.size.rm
}
# Combine all size
m.size.rm_dfs <- unlist(m.size.rm_dfs)

# Optionally, convert to a data frame for easier plotting
m.fitted_data.size.rm.pd <- data.frame(Temperature = rep(m.temp_seq, m.n.size.pd),
                                    Fitted = m.size.rm_dfs,
                                    Size = factor(rep(c("large", "medium", "small"), each = length(m.temp_seq))))
...
m.species.pd_to_size.rm <- unique(m.plot_data.size.pd[, c("Species", "Size")])  # Ensure species matches correct climate
...
ggsave("mosquito.size.rm.plot.png", plot = m.p.size.rm.pd, width = 8, height = 4)
...
save(m.size.pd.rm.fit,
     m.size.pd.rm.fit.DIC,
     file = "m.size.pd.rm.fit.exp.RData")
...
model {
  ...
  for (i in 1:m.N.obs.pd) {
    ...
    mu[i] <- mu.climate.a[m.climate.pd[i]] * exp(- mu.climate.Topt[m.climate.pd[i]] * temp[i]) + mu.size[m.size.pd[i]]
    trait[i] ~ dnorm(mu[i], size.tau[m.size.pd[i]])  T(0, )
  }}
...
# Get posterior samples
m.climate.size.samp <- as.matrix(m.climate.pd.size.fit.mcmc)
m.climate.size_vector <- numeric()
# Get column indices
a_m.climate.size <- grep("^mu\\.climate\\.a\\[", colnames(m.climate.size.samp))
Topt_m.climate.size <- grep("^mu\\.climate\\.Topt\\[", colnames(m.climate.size.samp))
c_m.climate.size <- grep("^mu\\.size\\[", colnames(m.climate.size.samp))
# Generate temperature sequence 
m.temp_seq <- seq(min(mosquito.pd$Temp), max(mosquito.pd$Temp), by = 0.1)
# Prepare a list to store data frames
m.climate.size_dfs <- list()
for(k in 1:m.n.climate.pd){
  for (j in 1:m.n.size.pd) {
    # Retrieve parameter vectors for species j
    m.a_samps <- m.climate.size.samp[, a_m.climate.size[k]]
    m.Topt_samps <- m.climate.size.samp[, Topt_m.climate.size[k]]
    m.c_samps <- m.climate.size.samp[, c_m.climate.size[j]]
    # For each temperature in temp_seq, evaluate across ALL posterior samples
    m.climate.size_eval <- sapply(m.temp_seq, function(temp) {
      m.a_samps * exp(- m.Topt_samps * temp) + m.c_samps
    })
    # Compute pointwise posterior means and HDIs
    m.climate.size.mean_curve <- colMeans(m.climate.size_eval) # length(temp_seq)
    m.climate.size_vector <- c(m.climate.size_vector, m.climate.size.mean_curve)
  }}
...
m.fitted_data.climate.size.pd <- data.frame(Temperature = rep(m.temp_seq, m.n.size.pd * m.n.climate.pd), 
                                            Fitted = m.climate.size_vector,
...
save(m.climate.pd.size.fit,
     m.climate.pd.size.fit.DIC,
     file = "m.climate.pd.size.fit.exp.RData")

rm(m.climate.pd.size.fit)

######################################### RMSE, DIC #########################################
df.aedes.pd.exp <- data.frame(
    Method = c("Aggregated Fit","By Species Individual",
               "By Species Hierarchical",
               "By Species Hierarchical, Different Variances",
               "By Climate", "By Wing Size",
               "By Wing Size and Climate"),
    DIC = c(m.one.data.pd.fit.DIC,
            m.DIC.species.individual.pd,
            m.species.pd.fit.DIC,
...
df.aedes.pd.exp$Delta.DIC <- df.aedes.pd.exp$DIC - m.DIC.species.individual.pd

m.method_levels <- df.aedes.pd.exp$Method[order(-df.aedes.pd.exp$DIC)]
df.aedes.pd.exp$Method <- factor(df.aedes.pd.exp$Method, levels = m.method_levels)

ggplot(df.aedes.pd.exp, aes(x = Delta.DIC, y = Method, fill = Method)) +
  ...
  theme(axis.title = element_text(size = 16),
        ...
        legend.position = "none")
ggsave("mosquito.DIC.png", width = 9, height = 7)


ggplot(df.aedes.pd.exp, aes(x = Total.RMSE, y = Method, fill = Method)) +
  geom_bar(stat = "identity", position = "dodge", color = "black") +
  ...
  theme(axis.title = element_text(size = 16),
        ...
        legend.title = element_text(size = 16),
        legend.position = "none")
ggsave("mosquito.RMSE.png", width = 9, height = 7)

save(df.aedes.pd.exp,
     file = "DIC.Mosquito.Exp.RData")
...
m.estimate.df <- data.frame(
  Model = as.factor(rep(c("Aggregated Fit","By Species Individual",
             "By Species Hierarchical",
             "By Species Hierarchical, Different Variances",
             "By Climate", "By Wing Size",
             "By Wing Size and Climate"), each = 2)),
  Species = as.factor(rep(c("Aedes aegypti", "Aedes notoscriptus"), times = 7)),
  Lower.HDI = c(hdi(m.one.data.pd.fit$BUGSoutput$sims.list$c)[1], 
                hdi(m.one.data.pd.fit$BUGSoutput$sims.list$c)[1],
                hdi(a.aegypti.pd.fit$BUGSoutput$sims.list$c)[1],
                hdi(a.notoscriptus.pd.fit$BUGSoutput$sims.list$c)[1],
                hdi(m.species.pd.fit$BUGSoutput$sims.list$mu.species[, 1])[1], 
                hdi(m.species.pd.fit$BUGSoutput$sims.list$mu.species[, 6])[1],
                hdi(m.species.pd.tau.fit$BUGSoutput$sims.list$mu.species[, 1])[1],
                hdi(m.species.pd.tau.fit$BUGSoutput$sims.list$mu.species[, 6])[1],
                hdi(m.climate.pd.fit$BUGSoutput$sims.list$mu.climate[, 3])[1],
                hdi(m.climate.pd.fit$BUGSoutput$sims.list$mu.climate[, 1])[1],
                rep(hdi(m.size.pd.fit$BUGSoutput$sims.list$mu.size[, 2])[1], 2),
                rep(hdi(m.climate.pd.size.fit$BUGSoutput$sims.list$mu.size[, 2])[1], 2)),
  Upper.HDI = c(hdi(m.one.data.pd.fit$BUGSoutput$sims.list$c)[2],
                hdi(m.one.data.pd.fit$BUGSoutput$sims.list$c)[2],
                hdi(a.aegypti.pd.fit$BUGSoutput$sims.list$c)[2],
                hdi(a.notoscriptus.pd.fit$BUGSoutput$sims.list$c)[2],
                hdi(m.species.pd.fit$BUGSoutput$sims.list$mu.species[, 1])[2], 
                hdi(m.species.pd.fit$BUGSoutput$sims.list$mu.species[, 6])[2],
                hdi(m.species.pd.tau.fit$BUGSoutput$sims.list$mu.species[, 1])[2],
                hdi(m.species.pd.tau.fit$BUGSoutput$sims.list$mu.species[, 6])[2],
                hdi(m.climate.pd.fit$BUGSoutput$sims.list$mu.climate[, 3])[2],
                hdi(m.climate.pd.fit$BUGSoutput$sims.list$mu.climate[, 1])[2],
                rep(hdi(m.size.pd.fit$BUGSoutput$sims.list$mu.size[, 2])[2], 2),
                rep(hdi(m.climate.pd.size.fit$BUGSoutput$sims.list$mu.size[, 2])[2], 2)),
  Median = c(median(m.one.data.pd.fit$BUGSoutput$sims.list$c),
             median(m.one.data.pd.fit$BUGSoutput$sims.list$c),
             median(a.aegypti.pd.fit$BUGSoutput$sims.list$c),
             median(a.notoscriptus.pd.fit$BUGSoutput$sims.list$c),
             median(m.species.pd.fit$BUGSoutput$sims.list$mu.species[, 1]), 
             median(m.species.pd.fit$BUGSoutput$sims.list$mu.species[, 6]),
             median(m.species.pd.tau.fit$BUGSoutput$sims.list$mu.species[, 1]),
             median(m.species.pd.tau.fit$BUGSoutput$sims.list$mu.species[, 6]),
             median(m.climate.pd.fit$BUGSoutput$sims.list$mu.climate[, 3]),
             median(m.climate.pd.fit$BUGSoutput$sims.list$mu.climate[, 1]),
             rep(median(m.size.pd.fit$BUGSoutput$sims.list$mu.size[, 2]), 2),
             rep(median(m.climate.pd.size.fit$BUGSoutput$sims.list$mu.size[, 2]), 2)))

m.estimate.df$Highlight <- ifelse(m.estimate.df$Model == "By Species Individual", "Highlight", "Normal")
m.estimate.df$Model <- factor(
  m.estimate.df$Model,
  levels = rev(unique(m.estimate.df$Model))  # keeps the order as-is
)
# Extract HDI bounds for "By Species Individual"
hdi_bounds <- m.estimate.df %>%
  filter(Model == "By Species Individual") %>%
  select(Species, Lower.HDI, Upper.HDI)

ggplot(m.estimate.df, aes(x = Model, y = Median, color = (Model == "By Species Individual"))) +
...
ggsave("Min.DT.png", height = 8, width = 14)
...
m.estimate.c.rm.df <- data.frame(
  Model = factor(rep(c("By Climate", 
                       "By Wing Size", 
                       "By Wize Size and Climate"), each = 4)),
  Species = as.factor(rep(c("Aedes aegypti", "Aedes krombeini"), times = 6)),
  Type = as.factor(rep(rep(c("Full", "Removed"), each = 2), 3)),
  Lower.HDI = c(rep(hdi(m.climate.pd.fit$BUGSoutput$sims.list$mu.climate[, 3])[1], 2),
                hdi(Climate.aegypti.pd.fit$BUGSoutput$sims.list$mu.climate[, 3])[1],
                hdi(Climate.krombeini.pd.fit$BUGSoutput$sims.list$mu.climate[, 3])[1],
                rep(hdi(m.size.pd.fit$BUGSoutput$sims.list$mu.size[, 2])[1], 2),
                hdi(Size.aegypti.pd.fit$BUGSoutput$sims.list$mu.size[, 2])[1],
                hdi(Size.krombeini.pd.fit$BUGSoutput$sims.list$mu.size[, 2])[1],
                rep(hdi(m.climate.pd.size.fit$BUGSoutput$sims.list$mu.size[, 2])[1], 2),
                hdi(m.climate.pd.Size.aegypti.fit$BUGSoutput$sims.list$mu.size[, 2])[1],
                hdi(m.climate.pd.Size.krombeini.fit$BUGSoutput$sims.list$mu.size[, 2])[1]),
  Upper.HDI = c(rep(hdi(m.climate.pd.fit$BUGSoutput$sims.list$mu.climate[, 3])[2], 2),
                hdi(Climate.aegypti.pd.fit$BUGSoutput$sims.list$mu.climate[, 3])[2],
                hdi(Climate.krombeini.pd.fit$BUGSoutput$sims.list$mu.climate[, 3])[2],
                rep(hdi(m.size.pd.fit$BUGSoutput$sims.list$mu.size[, 2])[2], 2),
                hdi(Size.aegypti.pd.fit$BUGSoutput$sims.list$mu.size[, 2])[2],
                hdi(Size.krombeini.pd.fit$BUGSoutput$sims.list$mu.size[, 2])[2],
                rep(hdi(m.climate.pd.size.fit$BUGSoutput$sims.list$mu.size[, 2])[2], 2),
                hdi(m.climate.pd.Size.aegypti.fit$BUGSoutput$sims.list$mu.size[, 2])[2],
                hdi(m.climate.pd.Size.krombeini.fit$BUGSoutput$sims.list$mu.size[, 2])[2]),
  Median = c(rep(median(m.climate.pd.fit$BUGSoutput$sims.list$mu.climate[, 3]), 2),
             median(Climate.aegypti.pd.fit$BUGSoutput$sims.list$mu.climate[, 3]),
             median(Climate.krombeini.pd.fit$BUGSoutput$sims.list$mu.climate[, 3]),
             rep(median(m.size.pd.fit$BUGSoutput$sims.list$mu.size[, 2]), 2),
             median(Size.aegypti.pd.fit$BUGSoutput$sims.list$mu.size[, 2]),
             median(Size.krombeini.pd.fit$BUGSoutput$sims.list$mu.size[, 2]),
             rep(median(m.climate.pd.size.fit$BUGSoutput$sims.list$mu.size[, 2]), 2),
             median(m.climate.pd.Size.aegypti.fit$BUGSoutput$sims.list$mu.size[, 2]),
             median(m.climate.pd.Size.krombeini.fit$BUGSoutput$sims.list$mu.size[, 2])))

m.estimate.c.rm.df$Model <- factor(
  m.estimate.c.rm.df$Model,
  levels = rev(unique(m.estimate.c.rm.df$Model))  # keeps the order as-is
)
ggplot(m.estimate.c.rm.df, aes(x = Model, y = Median, color = Type)) +
  geom_point(position = position_dodge(width = 0.5), size = 3) +
  geom_errorbar(aes(ymin = Lower.HDI, ymax = Upper.HDI), 
                position = position_dodge(width = 0.5), width = 0.2) +
  scale_color_manual(
    values = c("Removed" = "purple", "Full" = "black"),
...
ggsave("Min.Dev.RM.png", height = 4, width = 10)
...
m.fitted_data.a.aegypti.pd$Species <- "Aedes aegypti"
m.fitted_data.a.aegypti.pd$Fit <- "Individual Data"
m.fitted_data.a.albopictus.pd$Species <- "Aedes albopictus"
m.fitted_data.a.albopictus.pd$Fit <- "Individual Data"
m.fitted_data.a.camptorhynchus.pd$Species <- "Aedes camptorhynchus"
m.fitted_data.a.camptorhynchus.pd$Fit <- "Individual Data"
m.fitted_data.a.jj.pd$Species <- "Aedes japonicus japonicus"
m.fitted_data.a.jj.pd$Fit <- "Individual Data"
m.fitted_data.a.krombeini.pd$Species <- "Aedes krombeini"
m.fitted_data.a.krombeini.pd$Fit <- "Individual Data"
m.fitted_data.a.notoscriptus.pd$Species <- "Aedes notoscriptus"
m.fitted_data.a.notoscriptus.pd$Fit <- "Individual Data"
m.fitted_data.one.data.pd$Fit <- "Aggregated Fit"
m.fitted.full.ind <- rbind(m.fitted_data.one.data.pd,
...
ggplot() +
  geom_point(data = m.plot_data.one.data.pd, aes(x = Temperature, y = Trait), color = "grey52", alpha = 0.5) +
  geom_line(data = m.fitted.full.ind, aes(x = Temperature, y = Fitted, color = Fit), size = 1) +
  labs(x = "Temperature", y = "Development Time in Days") +
...
ggsave("mosquito.full.ind.plot.png", width = 11, height = 6)


m.fitted_data.species.tau.pd$Fit <- "By Species"
m.fitted.species.ind <- rbind(m.fitted_data.species.tau.pd,
                              m.fitted_data.a.aegypti.pd,
                              m.fitted_data.a.albopictus.pd,
                              m.fitted_data.a.camptorhynchus.pd,
                              m.fitted_data.a.jj.pd,
...
ggplot() +
  geom_point(data = m.plot_data.one.data.pd, aes(x = Temperature, y = Trait), color = "grey52", alpha = 0.5) +
  geom_line(data = m.fitted.species.ind, aes(x = Temperature, y = Fitted, color = Fit), size = 1) +
...
ggsave("mosquito.species.ind.plot.png", width = 11, height = 6)



m.fitted_data.a.aegypti.pd$`Climate` <- "Individual Data"
m.fitted_data.a.albopictus.pd$`Climate` <- "Individual Data"
m.fitted_data.a.camptorhynchus.pd$`Climate` <- "Individual Data"
m.fitted_data.a.jj.pd$`Climate` <- "Individual Data"
m.fitted_data.a.krombeini.pd$`Climate` <- "Individual Data"
m.fitted_data.a.notoscriptus.pd$`Climate` <- "Individual Data"
m.fitted.climate.ind <- rbind(m.fitted_data.climate.pd,
                              ...
                              m.fitted_data.a.krombeini.pd,
                              m.fitted_data.a.notoscriptus.pd)
m.fitted.climate.ind <- m.fitted.climate.ind |> 
  mutate(
    `Climate` = recode(Climate,
                  "temperate"  = "Temperatue",
                  "tropical" = "Tropical",
                  "subtropical"  = "Subtropical",
                  "Individual Data" = "Individual Data",
                  "Aggregated Fit" = "Aggregated Fit"),
    `Climate` = factor(`Climate`,
                  levels = c("Aggregated Fit", "Individual Data", "Temperatue", "Subtropical", "Tropical"))
  )

# Separate Individual vs. Climate and Size lines
m.individual_lines <- m.fitted.climate.ind %>% filter(`Climate` == "Individual Data")
m.clim_lines <- m.fitted.climate.ind %>% filter(!`Climate` %in% c("Individual Data", "Aggregated Fit"))
ggplot() +
  geom_point(data = m.plot_data.one.data.pd, aes(x = Temperature, y = Trait), color = "grey52", alpha = 0.5) +
  geom_line(data = m.individual_lines, aes(x = Temperature, y = Fitted, color = `Climate`), size = 1) +
  geom_line(data = m.full_lines, aes(x = Temperature, y = Fitted, color = `Climate`), size = 1) +
  geom_line(data = m.clim_lines, aes(x = Temperature, y = Fitted, color = `Climate`), size = 1) +
  labs(x = "Temperature", y = "Development Time in Days",
       color = "Fit") +
  scale_color_viridis_d(option = "viridis") +
  theme_minimal() +
...
ggsave("mosquito.climate.ind.plot.png", width = 11, height = 6)

m.full_lines <- m.fitted.climate.ind %>% filter(`Climate` == "Aggregated Fit")
ggplot() +
  geom_point(data = m.plot_data.one.data.pd, aes(x = Temperature, y = Trait), color = "grey52", alpha = 0.5) +
   # Fit type lines (individual + full) → separate legend for linetype
  geom_line(data = m.individual_lines, aes(x = Temperature, y = Fitted, linetype = "Species Individual"),
    color = "black", size = 1) +
  geom_line(data = m.full_lines, aes(x = Temperature, y = Fitted, linetype = "Aggregated Fit"),
    color = "red", size = 1) +
  # Climate fits → their own color legend
  geom_line(data = m.clim_lines, aes(x = Temperature, y = Fitted, color = Climate),
    size = 1) +
  # Separate legends
  scale_linetype_manual(name = "Fit",
    values = c("Species Individual" = "dotted", "Aggregated Fit" = "solid")) +
  scale_color_viridis_d(name = "Climate", option = "viridis") +
  labs(x = "Temperature", y = "Development Time in Days") +
  theme_minimal() +
  facet_wrap(~ Species) +
  theme(
...
ggsave("mosquito.climate.full.ind.plot.png", width = 11, height = 6)
...
m.fitted_data.a.notoscriptus.pd$`Climate` <- "Individual Data"
m.fitted.climate.ind <- rbind(m.fitted_data.climate.pd,
                              m.fitted_data.a.aegypti.pd[, - c(4, 6 ,7)],
                              m.fitted_data.a.albopictus.pd[, - c(4, 6 ,7)],
                              m.fitted_data.a.camptorhynchus.pd[, - c(4, 6 ,7)],
                              m.fitted_data.a.jj.pd[, - c(4, 6 ,7)],
                              m.fitted_data.a.krombeini.pd[, - c(4, 6 ,7)],
                              m.fitted_data.a.notoscriptus.pd[, - c(4, 6 ,7)])
m.fitted.climate.ind <- m.fitted.climate.ind |> 
  mutate(
    `Climate` = recode(Climate,
                       "temperate"  = "Temperatue",
                       ...
                       "Individual Data" = "Individual Data"),
    `Climate` = factor(`Climate`,
                       levels = c("Individual Data", "Temperatue", "Subtropical", "Tropical"))
  )

# Separate Individual vs. Climate and Size lines
m.individual_lines <- m.fitted.climate.ind %>% filter(`Climate` == "Individual Data")
m.clim_lines <- m.fitted.climate.ind %>% filter(`Climate` != "Individual Data")
ggplot() +
  geom_point(data = m.plot_data.one.data.pd, aes(x = Temperature, y = Trait), color = "grey52", alpha = 0.5) +
  geom_line(data = m.individual_lines, aes(x = Temperature, y = Fitted, color = `Climate`), size = 1) +
  geom_line(data = m.clim_lines, aes(x = Temperature, y = Fitted, color = `Climate`), size = 1) +
...
m.fitted_data.size.pd.plot <- m.fitted_data.size.pd
m.fitted_data.size.pd.plot$Size <- "Full Fit"
m.fitted.size.size.rm <- rbind(m.fitted_data.size.pd.plot,
                               m.fitted_data.size.rm.pd)
m.fitted.size.size.rm <- m.fitted.size.size.rm |> 
  mutate(
    Size = recode(Size,
                  "small"  = "Small",
                  "medium" = "Medium",
                  ...
                  "Full Fit" = "Full Fit"),
    Size = factor(Size,
                  levels = c("Full Fit", "Small", "Medium", "Large"))
  )
ggplot() +
  geom_point(data = m.plot_data.size.pd, aes(x = Temperature, y = Trait), color = "grey52", alpha = 0.5) +
  geom_line(data = m.fitted.size.size.rm, aes(x = Temperature, y = Fitted, color = Size), size = 1) +
  labs(x = "Temperature", y = "Development Time in Days") +
  scale_color_viridis_d(option = "viridis") +
  theme_minimal() +
  #theme(legend.position = "none") +
...
ggplot() + 
  ...
  geom_line(data = subset(m.fitted.size.size.rm, Size == "Full Fit"),
    aes(x = Temperature, y = Fitted, linetype = "Full Fit"), color = "red", size = 1) +
 # Size fits (separate color legend)
  geom_line(data = subset(m.fitted.size.size.rm, Size != "Full Fit"),
    aes(x = Temperature, y = Fitted, color = Size), size = 1) +
# Legends
  scale_linetype_manual(name = " ", values = c("Full Fit" = "solid")) +
  scale_color_viridis_d(name = "Size", option = "viridis") +
  labs(x = "Temperature", y = "Development Time in Days") +
  theme_minimal() +
  facet_wrap(~ Species) +
  theme(
...
ggsave("mosquito.size.size.rm.plot.png", width = 11, height = 6)
