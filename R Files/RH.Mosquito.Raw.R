...
ggplot() +
  ...
  labs(title = "Observed Data and Quadratic Fit by Species",
       x = "Temperature", y = "Development Time in Days") +
  theme_minimal() +
  facet_wrap(~ Species) 
...
m.fitted_data.climate.size$Combo <- with(m.fitted_data.climate.size, paste(Climate, Size, sep = ", "))
m.fitted_data.climate.size$Combo <- factor(m.fitted_data.climate.size$Combo, levels = c("temperate, large", 
...
ggplot() +
  ...
  geom_line(data = m.fitted_data.climate.size, aes(x = Temperature, y = Fitted, color = Combo), size = 1) +
  labs(
    title = "Observed Data and Quadratic Fit by Climate and Size",
    x = "Temperature",
