...
ggsave("Tick.Raw.png", width = 10, height = 10)
...
# Get rid of log liks
# full.data.raw.mcmc <- lapply(as.mcmc.list(full.data.fit.raw.mcmc), function(chain) {
#   chain[, !grepl("log_lik", colnames(chain))]
# })
# 
# # Run Gelman-Rubin diagnostic
# gelman.diag(as.mcmc.list(full.data.raw.mcmc), autoburnin = FALSE)
...
fitted_data.full.data.raw <- data.frame(Temperature = rep(temp_seq, n.species.raw), 
                                    ...
                                    Species = factor(rep(c("Amblyomma lepidum", "Dermacentor andersoni",
                                                           ...
                                                           "Hyalomma schulzei", "Ixodes anatis",
                                                           "Rhipicephalus (Boophilus) annulatus"), each = length(temp_seq))))
...
# Get rid of log liks
# species.raw.mcmc <- lapply(as.mcmc.list(species.raw.fit.mcmc), function(chain) {
#   chain[, !grepl("log_lik", colnames(chain))]
# })
# 
# # Run Gelman-Rubin diagnostic
# gelman.diag(as.mcmc.list(species.raw.mcmc), autoburnin = FALSE)
...
species10.raw <- function (x){
  a_species_mean.raw[10] * (x - Topt_species_mean.raw[10])^2 + c_species_mean.raw[10]
}
species11.raw <- function (x){
  a_species_mean.raw[11] * (x - Topt_species_mean.raw[11])^2 + c_species_mean.raw[11]
}
species12.raw <- function (x){
  a_species_mean.raw[12] * (x - Topt_species_mean.raw[12])^2 + c_species_mean.raw[12]
}
...
fitted_data.species.raw <- data.frame(Temperature = rep(temp_seq, n.species.raw), 
                                  Fitted = c(species1.raw(temp_seq),
                                             ...
                                             species10.raw(temp_seq),
                                             species11.raw(temp_seq),
                                             species12.raw(temp_seq)),
                                  Species = factor(rep(c("Amblyomma lepidum", "Dermacentor andersoni",
                                                         "Dermacentor nitens", "Haemaphysalis leporispalustris",
                                                         "Haemaphysalis longicornis",
                                                         "Hyalomma aegyptium", "Hyalomma dromedarii",
...
# Get rid of log liks
# species.tau.raw.mcmc <- lapply(as.mcmc.list(species.tau.raw.fit.mcmc), function(chain) {
#   chain[, !grepl("log_lik", colnames(chain))]
# })
# 
# # Run Gelman-Rubin diagnostic
# gelman.diag(as.mcmc.list(species.tau.raw.mcmc), autoburnin = FALSE)
...
species10.tau.raw <- function (x){
  a_species.tau_mean.raw[10] * (x - Topt_species.tau_mean.raw[10])^2 + c_species.tau_mean.raw[10]
}
species11.tau.raw <- function (x){
  a_species.tau_mean.raw[11] * (x - Topt_species.tau_mean.raw[11])^2 + c_species.tau_mean.raw[11]
}
species12.tau.raw <- function (x){
  a_species.tau_mean.raw[12] * (x - Topt_species.tau_mean.raw[12])^2 + c_species.tau_mean.raw[12]
}
...
fitted_data.species.tau.raw <- data.frame(Temperature = rep(temp_seq, n.species.raw), 
                                      Fitted = c(species1.tau.raw(temp_seq),
                                                 ...
                                                 species10.tau.raw(temp_seq),
                                                 species11.tau.raw(temp_seq),
                                                 species12.tau.raw(temp_seq)),
                                      Species = factor(rep(c("Amblyomma lepidum", "Dermacentor andersoni",
                                                             "Dermacentor nitens", "Haemaphysalis leporispalustris",
                                                             "Haemaphysalis longicornis",
                                                             "Hyalomma aegyptium", "Hyalomma dromedarii",
...
# Get rid of log liks
# species.tau.param.raw.mcmc <- lapply(as.mcmc.list(species.tau.param.raw.fit.mcmc), function(chain) {
#   chain[, !grepl("log_lik", colnames(chain))]
# })
# 
# # Run Gelman-Rubin diagnostic
# gelman.diag(as.mcmc.list(species.tau.param.raw.mcmc), autoburnin = FALSE)
...
# Get rid of log liks
# lepidum.raw.mcmc <- lapply(as.mcmc.list(a.lepidum.raw.fit.mcmc), function(chain) {
#   chain[, !grepl("log_lik", colnames(chain))]
# })
# 
# # Run Gelman-Rubin diagnostic
# gelman.diag(as.mcmc.list(lepidum.raw.mcmc), autoburnin = FALSE)
...
# # Get rid of log liks
# andersoni.raw.mcmc <- lapply(as.mcmc.list(d.andersoni.raw.fit.mcmc), function(chain) {
#   chain[, !grepl("log_lik", colnames(chain))]
# })
# 
# # Run Gelman-Rubin diagnostic
# gelman.diag(as.mcmc.list(andersoni.raw.mcmc), autoburnin = FALSE)
...
# Get rid of log liks
# nitens.raw.mcmc <- lapply(as.mcmc.list(d.nitens.raw.fit.mcmc), function(chain) {
#   chain[, !grepl("log_lik", colnames(chain))]
# })
# 
# # Run Gelman-Rubin diagnostic
# gelman.diag(as.mcmc.list(nitens.raw.mcmc), autoburnin = FALSE)
...
# Get rid of log liks
# leporispalustris.raw.mcmc <- lapply(as.mcmc.list(h.leporispalustris.raw.fit.mcmc), function(chain) {
#   chain[, !grepl("log_lik", colnames(chain))]
# })
# 
# # Run Gelman-Rubin diagnostic
# gelman.diag(as.mcmc.list(leporispalustris.raw.mcmc), autoburnin = FALSE)
...
################################### By Species Individual- Longicornis ###################################
h.longicornis.raw <- tick.raw[tick.raw$Species == "Haemaphysalis longicornis", ]

N.obs.ind.raw <- length(h.longicornis.raw$Trait)
...
# Bundle all data in a list for JAGS
jag.data <- list(trait = h.longicornis.raw$Trait, N.obs.ind.raw = N.obs.ind.raw, 
                 temp = h.longicornis.raw$Temp)

# Fit fill model
h.longicornis.raw.fit <- jags(data = jag.data, inits = inits, parameters.to.save = parameters, 
                                   model.file = "species_individual_robin.txt", n.thin = nt, n.chains = nc, n.burnin = nb, 
                                   n.iter = ni, DIC = T, working.directory = getwd(),
                              progress.bar = "gui" )
# Make mcmc object
h.longicornis.raw.fit.mcmc <- as.mcmc(h.longicornis.raw.fit) 
closeAllConnections()
#DIC
h.longicornis.raw.fit$BUGSoutput$DIC

# Get rid of log liks
# longicornis.raw.mcmc <- lapply(as.mcmc.list(h.longicornis.raw.fit.mcmc), function(chain) {
#   chain[, !grepl("log_lik", colnames(chain))]
# })
# 
# # Run Gelman-Rubin diagnostic
# gelman.diag(as.mcmc.list(longicornis.raw.mcmc), autoburnin = FALSE)

# Get posterior means
summary_fit.h.longicornis.raw <- summary(h.longicornis.raw.fit.mcmc)$statistics
a_mean.raw <- summary_fit.h.longicornis.raw["a", "Mean"]
c_mean.raw <- summary_fit.h.longicornis.raw["c", "Mean"]
Topt_mean.raw <- summary_fit.h.longicornis.raw["Topt", "Mean"]
...
# Data frame of temperature, fitted value, species
fitted_data.h.longicornis.raw <- data.frame(Temperature = rep(temp_seq),
                                                 Fitted = h.schulzei.fitted.raw(temp_seq))
# Original data with temperature, trait, and species as numeric
plot_data.h.longicornis.raw <- data.frame(
  Temperature = h.longicornis.raw$Temp,
  Trait = h.longicornis.raw$Trait
)

# Plot observed data and fitted curves 
p.h.longicornis.raw <- ggplot() +
  geom_point(data = plot_data.h.longicornis.raw, aes(x = Temperature, y = Trait), alpha = 0.5) +
  geom_line(data = fitted_data.h.longicornis.raw, aes(x = Temperature, y = Fitted), color = "seagreen1", size = 1) +
  labs(title = "Observed Data and Quadratic Fit",
       x = "Temperature", y = "Oviposition Time in Days") +
  theme_minimal() +
  theme(legend.position = "none")
...
# Merge the observed and predicted data for comparison
merged_data.h.longicornis.raw <- fitted_data.h.longicornis.raw %>%
  left_join(h.longicornis.raw, by = c("Temperature" = "Temp"))

# Calculate RMSE for each species
rmse.h.longicornis.raw <- merged_data.h.longicornis.raw %>%
  summarize(
    MSE = mean((Trait - Fitted)^2, na.rm = TRUE),
    SE = (Trait - Fitted)^2 
  )
...
# Extract log-likelihood matrix (samples × observations)
log.lik.h.longicornis.raw <- h.longicornis.raw.fit$BUGSoutput$sims.list$log_lik
# Compute WAIC
waic.h.longicornis.raw <- waic(log.lik.h.longicornis.raw)$estimates["waic", "Estimate"]


save(rmse.h.longicornis.raw,
     fitted_data.h.longicornis.raw,
     merged_data.h.longicornis.raw,
     h.longicornis.raw.fit.mcmc,
     h.longicornis.raw.fit,
     p.h.longicornis.raw,
     waic.h.longicornis.raw,
     file="h.longicornis.raw.fit.RData")
...
# Get rid of log liks
# aegyptium.raw.mcmc <- lapply(as.mcmc.list(h.aegyptium.raw.fit.mcmc), function(chain) {
#   chain[, !grepl("log_lik", colnames(chain))]
# })
# 
# # Run Gelman-Rubin diagnostic
# gelman.diag(as.mcmc.list(aegyptium.raw.mcmc), autoburnin = FALSE)
...
# Get rid of log liks
# dromedarii.raw.mcmc <- lapply(as.mcmc.list(h.dromedarii.raw.fit.mcmc), function(chain) {
#   chain[, !grepl("log_lik", colnames(chain))]
# })
# 
# # Run Gelman-Rubin diagnostic
# gelman.diag(as.mcmc.list(dromedarii.raw.mcmc), autoburnin = FALSE)
...
# Get rid of log liks
# impeltatum.raw.mcmc <- lapply(as.mcmc.list(h.impeltatum.raw.fit.mcmc), function(chain) {
#   chain[, !grepl("log_lik", colnames(chain))]
# })
# 
# # Run Gelman-Rubin diagnostic
# gelman.diag(as.mcmc.list(impeltatum.raw.mcmc), autoburnin = FALSE)
...
# # Get rid of log liks
# lusitanicum.raw.mcmc <- lapply(as.mcmc.list(h.lusitanicum.raw.fit.mcmc), function(chain) {
#   chain[, !grepl("log_lik", colnames(chain))]
# })
# 
# # Run Gelman-Rubin diagnostic
# gelman.diag(as.mcmc.list(lusitanicum.raw.mcmc), autoburnin = FALSE)
...
# Get rid of log liks
# schulzei.raw.mcmc <- lapply(as.mcmc.list(h.schulzei.raw.fit.mcmc), function(chain) {
#   chain[, !grepl("log_lik", colnames(chain))]
# })
# 
# # Run Gelman-Rubin diagnostic
# gelman.diag(as.mcmc.list(schulzei.raw.mcmc), autoburnin = FALSE)
...
################################### By Species Individual- Anatis ###################################
i.anatis.raw <- tick.raw[tick.raw$Species == "Ixodes anatis", ]

N.obs.ind.raw <- length(i.anatis.raw$Trait)
...
# Bundle all data in a list for JAGS
jag.data <- list(trait = i.anatis.raw$Trait, N.obs.ind.raw = N.obs.ind.raw, 
                 temp = i.anatis.raw$Temp)

# Fit fill model
i.anatis.raw.fit <- jags(data = jag.data, inits = inits, parameters.to.save = parameters, 
                           model.file = "species_individual_robin.txt", n.thin = nt, n.chains = nc, n.burnin = nb, 
                           n.iter = ni, DIC = T, working.directory = getwd())
# Make mcmc object
i.anatis.raw.fit.mcmc <- as.mcmc(i.anatis.raw.fit) 
closeAllConnections()
#DIC
i.anatis.raw.fit$BUGSoutput$DIC

# Get rid of log liks
# anatis.raw.mcmc <- lapply(as.mcmc.list(i.anatis.raw.fit.mcmc), function(chain) {
#   chain[, !grepl("log_lik", colnames(chain))]
# })
# 
# # Run Gelman-Rubin diagnostic
# gelman.diag(as.mcmc.list(anatis.raw.mcmc), autoburnin = FALSE)

# Get posterior means
summary_fit.i.anatis.raw <- summary(i.anatis.raw.fit.mcmc)$statistics
a_mean.raw <- summary_fit.i.anatis.raw["a", "Mean"]
c_mean.raw <- summary_fit.i.anatis.raw["c", "Mean"]
Topt_mean.raw <- summary_fit.i.anatis.raw["Topt", "Mean"]

# Generate temperature sequence 
temp_seq <- seq(min(tick.raw$Temp), max(tick.raw$Temp), by = 0.1)
# 
# # Get predicted values
i.anatis.fitted.raw <- function (x){
  a_mean.raw * (x - Topt_mean.raw)^2 + c_mean.raw
}


# Data frame of temperature, fitted value, species
fitted_data.i.anatis.raw <- data.frame(Temperature = rep(temp_seq),
                                         Fitted = i.anatis.fitted.raw(temp_seq))
# Original data with temperature, trait, and species as numeric
plot_data.i.anatis.raw <- data.frame(
  Temperature = i.anatis.raw$Temp,
  Trait = i.anatis.raw$Trait
)

# Plot observed data and fitted curves 
p.i.anatis.raw <- ggplot() +
  geom_point(data = plot_data.i.anatis.raw, aes(x = Temperature, y = Trait), alpha = 0.5) +
  geom_line(data = fitted_data.i.anatis.raw, aes(x = Temperature, y = Fitted), color = "seagreen1", size = 1) +
  labs(title = "Observed Data and Quadratic Fit",
       x = "Temperature", y = "Oviposition Time in Days") +
  theme_minimal() +
  theme(legend.position = "none")
...
# Merge the observed and predicted data for comparison
merged_data.i.anatis.raw <- fitted_data.i.anatis.raw %>%
  left_join(i.anatis.raw, by = c("Temperature" = "Temp"))

# Calculate RMSE for each species
rmse.i.anatis.raw <- merged_data.i.anatis.raw %>%
  summarize(
    MSE = mean((Trait - Fitted)^2, na.rm = TRUE),
    SE = (Trait - Fitted)^2 
  )
...
# Extract log-likelihood matrix (samples × observations)
log.lik.i.anatis.raw <- i.anatis.raw.fit$BUGSoutput$sims.list$log_lik
# Compute WAIC
waic.i.anatis.raw <- waic(log.lik.i.anatis.raw)$estimates["waic", "Estimate"]


save(rmse.i.anatis.raw,
     fitted_data.i.anatis.raw,
     merged_data.i.anatis.raw,
     i.anatis.raw.fit.mcmc,
     i.anatis.raw.fit,
     p.i.anatis.raw,
     waic.i.anatis.raw,
     file="i.anatis.raw.fit.RData")

################################### By Species Individual- Annulatus ###################################
r.annulatus.raw <- tick.raw[tick.raw$Species == "Rhipicephalus (Boophilus) annulatus", ]

N.obs.ind.raw <- length(r.annulatus.raw$Trait)
...
# Bundle all data in a list for JAGS
jag.data <- list(trait = r.annulatus.raw$Trait, N.obs.ind.raw = N.obs.ind.raw, 
                 temp = r.annulatus.raw$Temp)

# Fit fill model
r.annulatus.raw.fit <- jags(data = jag.data, inits = inits, parameters.to.save = parameters, 
                           model.file = "species_individual_robin.txt", n.thin = nt, n.chains = nc, n.burnin = nb, 
                           n.iter = ni, DIC = T, working.directory = getwd())
# Make mcmc object
r.annulatus.raw.fit.mcmc <- as.mcmc(r.annulatus.raw.fit) 
closeAllConnections()
#DIC
r.annulatus.raw.fit$BUGSoutput$DIC

# Get rid of log liks
# annulatus.raw.mcmc <- lapply(as.mcmc.list(r.annulatus.raw.fit.mcmc), function(chain) {
#   chain[, !grepl("log_lik", colnames(chain))]
# })
# 
# # Run Gelman-Rubin diagnostic
# gelman.diag(as.mcmc.list(annulatus.raw.mcmc), autoburnin = FALSE)

# Get posterior means
summary_fit.r.annulatus.raw <- summary(r.annulatus.raw.fit.mcmc)$statistics
a_mean.raw <- summary_fit.r.annulatus.raw["a", "Mean"]
c_mean.raw <- summary_fit.r.annulatus.raw["c", "Mean"]
Topt_mean.raw <- summary_fit.r.annulatus.raw["Topt", "Mean"]

# Generate temperature sequence 
temp_seq <- seq(min(tick.raw$Temp), max(tick.raw$Temp), by = 0.1)
# 
# # Get predicted values
r.annulatus.fitted.raw <- function (x){
  a_mean.raw * (x - Topt_mean.raw)^2 + c_mean.raw
}


# Data frame of temperature, fitted value, species
fitted_data.r.annulatus.raw <- data.frame(Temperature = rep(temp_seq),
                                         Fitted = r.annulatus.fitted.raw(temp_seq))
# Original data with temperature, trait, and species as numeric
plot_data.r.annulatus.raw <- data.frame(
  Temperature = r.annulatus.raw$Temp,
  Trait = r.annulatus.raw$Trait
)

# Plot observed data and fitted curves 
p.r.annulatus.raw <- ggplot() +
  geom_point(data = plot_data.r.annulatus.raw, aes(x = Temperature, y = Trait), alpha = 0.5) +
  geom_line(data = fitted_data.r.annulatus.raw, aes(x = Temperature, y = Fitted), color = "seagreen1", size = 1) +
  labs(title = "Observed Data and Quadratic Fit",
       x = "Temperature", y = "Oviposition Time in Days") +
  theme_minimal() +
  theme(legend.position = "none")
...
# Merge the observed and predicted data for comparison
merged_data.r.annulatus.raw <- fitted_data.r.annulatus.raw %>%
  left_join(r.annulatus.raw, by = c("Temperature" = "Temp"))

# Calculate RMSE for each species
rmse.r.annulatus.raw <- merged_data.r.annulatus.raw %>%
  summarize(
    MSE = mean((Trait - Fitted)^2, na.rm = TRUE),
    SE = (Trait - Fitted)^2 
  )
...
# Extract log-likelihood matrix (samples × observations)
log.lik.r.annulatus.raw <- r.annulatus.raw.fit$BUGSoutput$sims.list$log_lik
# Compute WAIC
waic.r.annulatus.raw <- waic(log.lik.r.annulatus.raw)$estimates["waic", "Estimate"]


save(rmse.r.annulatus.raw,
     fitted_data.r.annulatus.raw,
     merged_data.r.annulatus.raw,
     r.annulatus.raw.fit.mcmc,
     r.annulatus.raw.fit,
     p.r.annulatus.raw,
     waic.r.annulatus.raw,
     file="r.annulatus.raw.fit.RData")
...
waic.species.ind.raw <- waic(log.lik.a.lepidum.raw)$estimates["waic", "Estimate"] + 
  ...
  waic(log.lik.h.longicornis.raw)$estimates["waic", "Estimate"] +
  waic(log.lik.i.anatis.raw)$estimates["waic", "Estimate"] +
  waic(log.lik.r.annulatus.raw)$estimates["waic", "Estimate"]
...
DIC.species.individual.raw <- a.lepidum.raw.fit$BUGSoutput$DIC + d.andersoni.raw.fit$BUGSoutput$DIC + 
  ...
  h.schulzei.raw.fit$BUGSoutput$DIC + h.longicornis.raw.fit$BUGSoutput$DIC +
  i.anatis.raw.fit$BUGSoutput$DIC + r.annulatus.raw.fit$BUGSoutput$DIC
...
se.individual.raw <- c(rmse.a.lepidum.raw$SE, rmse.d.andersoni.raw$SE, rmse.d.nitens.raw$SE,
                       ...
                       rmse.h.longicornis.raw$SE, rmse.i.anatis.raw$SE,
                       rmse.r.annulatus.raw$SE)
...
grid.ind.species.raw <- gridExtra::grid.arrange(
  ...
  p.h.leporispalustris.raw + labs(title = "Haemaphysalis leporispalustris", x = " "),
  p.h.longicornis.raw + labs(title = "Haemaphysalis longicornus", x = " ", y = "Oviposition in Days"),
  p.h.aegyptium.raw + labs(title = "Hyalomma aegyptium", x = " ", y = " "),
  p.h.dromedarii.raw + labs(title = "Hyalomma dromedarii", x = " ", y = " "),
  p.h.impeltatum.raw + labs(title = "Hyalomma impeltatum", y = " "),
  p.h.lusitanicum.raw + labs(title = "Hyalomma lusitanicum", y = "Oviposition in Days"),
  p.h.schulzei.raw + labs(title = "Hyalomma schulzei", y = " "),
  p.i.anatis.raw + labs(title = "Ixodes anatis", y = " "),
  p.r.annulatus.raw + labs(title = "Rhipicephalus (Boophilus) annulatus", y = " "),
  nrow = 3, ncol = 4
)
...
# Get rid of log liks
# genus.raw.mcmc <- lapply(as.mcmc.list(genus.raw.fit.mcmc), function(chain) {
#   chain[, !grepl("log_lik", colnames(chain))]
# })
# 
# # Run Gelman-Rubin diagnostic
# gelman.diag(as.mcmc.list(genus.raw.mcmc), autoburnin = FALSE)
...
genus5.raw <- function (x){
  a_genus_mean.raw[5] * (x - Topt_genus_mean.raw[5])^2 + c_genus_mean.raw[5]
}
genus6.raw <- function (x){
  a_genus_mean.raw[6] * (x - Topt_genus_mean.raw[6])^2 + c_genus_mean.raw[6]
}
...
fitted_data.genus.raw <- data.frame(Temperature = rep(temp_seq, n.genus.raw), 
                                ...
                                Genus = factor(rep(c("Amblyomma", "Dermacentor",
                                                     ...
                                                     "Ixodes", "Rhipicephalus"), each = length(temp_seq))))
...
# Get rid of log liks
# climate.raw.mcmc <- lapply(as.mcmc.list(climate.raw.fit.mcmc), function(chain) {
#   chain[, !grepl("log_lik", colnames(chain))]
# })
# 
# # Run Gelman-Rubin diagnostic
# gelman.diag(as.mcmc.list(climate.raw.mcmc), autoburnin = FALSE)
...
# Get rid of log liks
# host.raw.mcmc <- lapply(as.mcmc.list(host.raw.fit.mcmc), function(chain) {
#   chain[, !grepl("log_lik", colnames(chain))]
# })
# 
# # Run Gelman-Rubin diagnostic
# gelman.diag(as.mcmc.list(host.raw.mcmc), autoburnin = FALSE)
...
# 
# # Get rid of log liks
# size.raw.mcmc <- lapply(as.mcmc.list(size.raw.fit.mcmc), function(chain) {
#   chain[, !grepl("log_lik", colnames(chain))]
# })
# 
# # Run Gelman-Rubin diagnostic
# gelman.diag(as.mcmc.list(size.raw.mcmc), autoburnin = FALSE)
...
# Get rid of log liks
# continent.raw.mcmc <- lapply(as.mcmc.list(continent.raw.fit.mcmc), function(chain) {
#   chain[, !grepl("log_lik", colnames(chain))]
# })
# 
# # Run Gelman-Rubin diagnostic
# gelman.diag(as.mcmc.list(continent.raw.mcmc), autoburnin = FALSE)
...
# Get rid of log liks
# host.climate.raw.mcmc <- lapply(as.mcmc.list(host.climate.raw.fit.mcmc), function(chain) {
#   chain[, !grepl("log_lik", colnames(chain))]
# })
# 
# # Run Gelman-Rubin diagnostic
# gelman.diag(as.mcmc.list(host.climate.raw.mcmc), autoburnin = FALSE)
...
fitted_data.host.climate.raw$`Host, Climate` <- with(fitted_data.host.climate.raw, paste(Host, Climate, sep = ", "))
fitted_data.host.climate.raw$`Host, Climate` <- factor(fitted_data.host.climate.raw$`Host, Climate`, levels = c("One, Mixed", "One, Tropical", 
...
ggplot() +
  ...
  geom_line(data = fitted_data.host.climate.raw, aes(x = Temperature, y = Fitted, color = `Host, Climate`), size = 1) +
  labs(x = "Temperature", y = "Oviposition Time in Days") +
  theme_minimal() +
  facet_wrap(~ Species) + 
  theme(
...
# # Get rid of log liks
# host.genus.raw.mcmc <- lapply(as.mcmc.list(host.genus.raw.fit.mcmc), function(chain) {
#   chain[, !grepl("log_lik", colnames(chain))]
# })
# 
# # Run Gelman-Rubin diagnostic
# gelman.diag(as.mcmc.list(host.genus.raw.mcmc), autoburnin = FALSE)
...
host1genus5.raw <- function (x){
  a_host.genus_mean.raw[1] * (x - Topt_host.genus_mean.raw[5])^2 + c_host.genus_mean.raw[5]
}
host2genus5.raw <- function (x){
  a_host.genus_mean.raw[2] * (x - Topt_host.genus_mean.raw[5])^2 + c_host.genus_mean.raw[5]
}
host3genus5.raw <- function (x){
  a_host.genus_mean.raw[3] * (x - Topt_host.genus_mean.raw[5])^2 + c_host.genus_mean.raw[5]
}
host1genus6.raw <- function (x){
  a_host.genus_mean.raw[1] * (x - Topt_host.genus_mean.raw[6])^2 + c_host.genus_mean.raw[6]
}
host2genus6.raw <- function (x){
  a_host.genus_mean.raw[2] * (x - Topt_host.genus_mean.raw[6])^2 + c_host.genus_mean.raw[6]
}
host3genus6.raw <- function (x){
  a_host.genus_mean.raw[3] * (x - Topt_host.genus_mean.raw[6])^2 + c_host.genus_mean.raw[6]
}
...
fitted_data.host.genus.raw <- data.frame(Temperature = rep(temp_seq, n.host.raw * n.genus.raw), 
                                     Fitted = c(host1genus1.raw(temp_seq),
                                                ...
                                                host1genus5.raw(temp_seq),
                                                host2genus5.raw(temp_seq),
                                                host3genus5.raw(temp_seq),
                                                host1genus6.raw(temp_seq),
                                                host2genus6.raw(temp_seq),
                                                host3genus6.raw(temp_seq)),
...
fitted_data.host.genus.raw$`Host, Genus` <- with(fitted_data.host.genus.raw, paste(Host, Genus, sep = ", "))
fitted_data.host.genus.raw$`Host, Genus` <- factor(fitted_data.host.genus.raw$`Host, Genus`, levels = c("One, Dermacentor",
                                                                                                        "One, Rhipicephalus",
                                                                                "Two, Hyalomma", "Three, Amblyomma",
                                                                                "Three, Dermacentor", "Three, Haemaphysalis",
                                                                                "Three, Hyalomma", "Three, Ixodes"))

# Plot observed data and fitted curves with facet wrap by species
ggplot() +
  geom_point(data = plot_data.host.genus.raw, aes(x = Temperature, y = Trait), alpha = 0.5) +
  geom_line(data = fitted_data.host.genus.raw, aes(x = Temperature, y = Fitted, color = `Host, Genus`), size = 1) +
  labs(x = "Temperature", y = "Oviposition Time in Days") +
  theme_minimal() +
  facet_wrap(~ Species) + 
  theme(
...
# Get rid of log liks
# genus.climate.raw.mcmc <- lapply(as.mcmc.list(genus.climate.raw.fit.mcmc), function(chain) {
#   chain[, !grepl("log_lik", colnames(chain))]
# })
# 
# # Run Gelman-Rubin diagnostic
# gelman.diag(as.mcmc.list(genus.climate.raw.mcmc), autoburnin = FALSE)
# A few Topts upper CI above 1.1
...
genus5clim1.raw <- function (x){
  a_genus.climate_mean.raw[1] * (x - Topt_genus.climate_mean.raw[5])^2 + c_genus.climate_mean.raw[5]
}
genus6clim1.raw <- function (x){
  a_genus.climate_mean.raw[1] * (x - Topt_genus.climate_mean.raw[6])^2 + c_genus.climate_mean.raw[6]
}
...
genus5clim2.raw <- function (x){
  a_genus.climate_mean.raw[2] * (x - Topt_genus.climate_mean.raw[5])^2 + c_genus.climate_mean.raw[5]
}
genus6clim2.raw <- function (x){
  a_genus.climate_mean.raw[2] * (x - Topt_genus.climate_mean.raw[6])^2 + c_genus.climate_mean.raw[6]
}
...
genus5clim3.raw <- function (x){
  a_genus.climate_mean.raw[3] * (x - Topt_genus.climate_mean.raw[5])^2 + c_genus.climate_mean.raw[5]
}
genus6clim3.raw <- function (x){
  a_genus.climate_mean.raw[3] * (x - Topt_genus.climate_mean.raw[6])^2 + c_genus.climate_mean.raw[6]
}
...
genus5clim4.raw <- function (x){
  a_genus.climate_mean.raw[4] * (x - Topt_genus.climate_mean.raw[5])^2 + c_genus.climate_mean.raw[5]
}
genus6clim4.raw <- function (x){
  a_genus.climate_mean.raw[4] * (x - Topt_genus.climate_mean.raw[6])^2 + c_genus.climate_mean.raw[6]
}
...
# Combine Climate and Size for coloring
fitted_data.genus.climate.raw$`Genus, Climate` <- with(fitted_data.genus.climate.raw, paste(Genus, Climate, sep = ", "))


# Plot observed data and fitted curves with facet wrap by species
ggplot() +
  geom_point(data = plot_data.genus.climate.raw, aes(x = Temperature, y = Trait), alpha = 0.5) +
  geom_line(data = fitted_data.genus.climate.raw, aes(x = Temperature, y = Fitted, color = `Genus, Climate`), size = 1) +
...
# # Get rid of log liks
# h.c.s.raw.mcmc <- lapply(as.mcmc.list(h.c.s.raw.fit.mcmc), function(chain) {
#   chain[, !grepl("log_lik", colnames(chain))]
# })
# 
# # Run Gelman-Rubin diagnostic
# gelman.diag(as.mcmc.list(h.c.s.raw.mcmc), autoburnin = FALSE)
...
fitted_data.h.c.s.raw$`Host, Climate, Size` <- with(fitted_data.h.c.s.raw, paste(Host, Climate, Size, sep = ", "))
fitted_data.h.c.s.raw$`Host, Climate, Size` <- factor(fitted_data.h.c.s.raw$`Host, Climate, Size`, levels = c("One, Tropical, Medium",
...
ggplot() +
  ...
  geom_line(data = fitted_data.h.c.s.raw, aes(x = Temperature, y = Fitted, color = `Host, Climate, Size`), size = 1) +
  labs(x = "Temperature", y = "Oviposition Time in Days") +
  theme_minimal() +
  facet_wrap(~ Species) + 
  theme(
...
# Get rid of log liks
# h.c.g.raw.mcmc <- lapply(as.mcmc.list(h.c.g.raw.fit.mcmc), function(chain) {
#   chain[, !grepl("log_lik", colnames(chain))]
# })
# 
# # Run Gelman-Rubin diagnostic
# gelman.diag(as.mcmc.list(h.c.g.raw.mcmc), autoburnin = FALSE)
...
# Combine Host, Climate, and Size for coloring
fitted_data.h.c.g.raw$`Host, Climate, Genus` <- with(fitted_data.h.c.g.raw, paste(Host, Climate, Genus, sep = ", "))

# Plot observed data and fitted curves with facet wrap by species
ggplot() +
  geom_point(data = plot_data.h.c.g.raw, aes(x = Temperature, y = Trait), alpha = 0.5) +
  geom_line(data = fitted_data.h.c.g.raw, aes(x = Temperature, y = Fitted, color = `Host, Climate, Genus`), size = 1) +
...
ggplot(table.tick.raw, aes(x = Method, y = DIC, fill = Method)) +
  ...
  labs(title = "DIC by Method", x = "Method",
       y = "Deviance Information Criterion (RMSE)") +
  theme_minimal() +
